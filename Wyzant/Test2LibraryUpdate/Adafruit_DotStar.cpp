...
long _dotStarSpeed = 8000000;
//long _dotStarSpeed = 2000000;
...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t o)
  ...
  spi_dev = new Adafruit_SPIDevice(-1, _dotStarSpeed);
  updateLength(n);
}
...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t data, uint8_t clock,
    ...
    : brightness(0), pixels(NULL), rOffset(o & 3), gOffset((o >> 2) & 3),
      bOffset((o >> 4) & 3) {
  spi_dev = new Adafruit_SPIDevice(-1, clock, -1, data, _dotStarSpeed);
  updateLength(n);
}
