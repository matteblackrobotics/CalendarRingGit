...
// -------------------- stepTo ------------------- //
// step towards target if above threshold, cannot be greater than threshold
void stepTo(float current, float target, float step, float threshold)
{
  if(step > threshold)
  {
    Serial.println();
    Serial.print("Error: step must be smaller than threshold");
    Serial.println();
  }
  else
  {
    ...
    {
      if(current < target) {current = current + step;}
      if(current > target) {current = current - step;}
    } 
  }
}
